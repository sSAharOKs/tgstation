...
/datum/team/revolution/proc/process_victory(revs_win_injection_amount)
	...
	if (. == STATION_VICTORY)
		...
		priority_announce("Похоже, мятеж был подавлен. Пожалуйста, верните себя и своих недееспособных коллег к работе. \
		Мы дистанционно занесли в черный список главных революционеров в вашей медицинской карте, чтобы предотвратить случайное повторение.", null, null, null, "Отдел контроля лояльности Центрального командования")
	else
		...
		priority_announce("Недавняя оценка вашей станции отметила её как зону серьезного риска для высокопоставленных чиновников Нанотразена. \
		Для безопасности наших сотрудников мы внесли вашу станцию в черный список для нового назначения на службу безопасности и командования. \
		[pick(world.file2list("strings/anti_union_propaganda.txt"))]", null, null, null, "Отдел контроля лояльности Центрального командования")
