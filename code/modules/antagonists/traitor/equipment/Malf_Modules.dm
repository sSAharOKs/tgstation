...
/datum/action/innate/ai/nuke_station/proc/set_up_us_the_bomb(mob/living/owner)
	...
	if (owner_AI.stat != DEAD)
		priority_announce("Во всех системах станций обнаружены враждебные среды выполнения, пожалуйста, отключите свой ИИ, чтобы предотвратить возможное повреждение его морального ядра.", "Предупреждение об аномалии", ANNOUNCER_AIMALF)
...
/obj/machinery/doomsday_device/process()
	...
	if(!T || !is_station_level(T.z))
		minor_announce("УСТРОЙСТВО СУДНОГО ДНЯ ВНЕ ЗОНЫ ДЕЙСТВИЯ СТАНЦИИ, ПРЕРЫВАНИЕ", "ОШИБКА 0ШИБKА 0И0ШИБkА$!R41.%%!!(%$^^__+ @#F0E4", TRUE)
	...
	if(!sec_left)
		...
		minor_announce("[sec_left] СЕКУНД ДО АКТИВАЦИИ УСТРОЙСТВА СУДНОГО ДНЯ!", "ОШИБКА 0ШИБKА 0И0ШИБkА$!R41.%%!!(%$^^__+ @#F0E4", TRUE)
...
/datum/action/innate/ai/lockdown/Activate()
	...
	minor_announce("В дверных контроллерах обнаружена вредоносная программа. В настоящее время действуют протоколы изоляции и блокировки. Пожалуйста, сохраняйте спокойствие.","Сетевое оповещение:", TRUE)
