...
/datum/round_event/grey_tide/announce(fake)
	if(areasToOpen && areasToOpen.len > 0)
		priority_announce("Вирус Асэс.ТЭ#nT обнаружен в дверных подпрограммах на [station_name()]. Уровень серьезности [severity]. Рекомендовано участие ИИ станции.", "Предупреждение о безопасности")
	else
