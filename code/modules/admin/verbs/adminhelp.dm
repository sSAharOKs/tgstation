...
/datum/admin_help/proc/Close(key_name = key_name_admin(usr), silent = FALSE)
	...
	if(!silent)
		...
		SSredbot.send_discord_message("admin", "Тикет #[id] закрыт администратором [usr.key]", "ticket")
		message_admins(msg)
...
/datum/admin_help/proc/Reject(key_name = key_name_admin(usr))
	...
	SSredbot.send_discord_message("admin", "Тикет #[id] отклонен администратором [usr.key]", "ticket")
	message_admins(msg)
	log_admin_private(msg)
